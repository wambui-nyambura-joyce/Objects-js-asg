...
const people = [
  ...
  for (const person of people) {
    if (person.age >= 18) {
      namesOfAdults.push(person.name);
    }
  }
...
// and category property. The function should return an object that groups the products by their categories, with 
// the category names as keys and the arrays of products as values.
// const products = [
...
// ];
  ...
  function getStudentsWithAverageScoreAboveThreshold(students, threshold) {
    const studentsWithAverageAboveThreshold = [];
    for (const student of students) {
      const averageScore = student.scores.reduce((acc, score) => acc + score, 0) / student.scores.length;
      if (averageScore >= threshold) {
        studentsWithAverageAboveThreshold.push(student.name);
      }
    }
    return studentsWithAverageAboveThreshold;
  }
  

  //Question3
//   Given an array of objects, where each object represents a student with a name and an array of scores, write
//  a function that returns a new array containing the names of all students whose average score is greater than 
// or equal to 85.
//   const students = [
...
//   ];
  ...
  console.log(getStudentsWithAverageScoreAboveThreshold(students, 85)); // Output: ["John", "Jane", "Jill"]
